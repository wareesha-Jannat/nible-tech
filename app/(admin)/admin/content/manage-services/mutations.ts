...
type UpdatePriorityInput = { id: string; priority: number };

export function useUpdateServicePriorityMutation() {
  const queryClient = useQueryClient();

  const mutation = useMutation<ServiceItem, Error, UpdatePriorityInput>({
    mutationFn: async ({ id, priority }: UpdatePriorityInput) => {
    ...
    onSuccess: async (updated: ServiceItem) => {
      ...
      queryClient.setQueriesData(
        ...
        (oldData: InfiniteData<ServicesPage> | undefined) => {
          ...
          return {
            ...oldData,
            pages: oldData.pages.map((page: ServicesPage) => ({
              ...page,
              services: page.services.map((s: ServiceItem) =>
